...
def Landing():
    ...
    button1 = CTkButton(frame, text = "Login",font = ("Times", 75, "bold"),fg_color="transparent",text_color = "#a51212",border_width = 2, border_color = "blue",corner_radius = 12, command = login1234).place(x = framex / 2- 200 - 50 + 50 + 100, y =150)
    button2 = CTkButton(frame, text = "Sign Up",font = ("Times", 75, "bold"),fg_color="transparent",text_color = "#a51212",border_width = 2, border_color = "blue",corner_radius = 12, command = signup).place(x = framex / 2- 200 - 50 + 50 + 100 - 35, y = framey - 300 - 50 + 25)
    button3 = CTkButton(frame, text = "Quit",font = ("Times", 75, "bold"),fg_color="transparent",text_color = "#a51212",border_width = 2, border_color = "blue",corner_radius = 12, command = quitoff).place(x = framex / 2- 200 - 50 + 50 + 100 + 15, y = framey - 150)
...
def SignUp():
    ...
    background = CTkImage(light_image = Image.open('Default Stuff\\Wallpaper.jpg'),
                        size=(w, h))
    back = CTkLabel(win3,text = ' ', image = background).pack()
    frame = CTkFrame(win3,fg_color="white",width=w - 121,height=h - 101)
    frame.place(x = w / 2 - (w - 121) / 2, y = h / 2 - (h - 101) / 2)
    label2 = CTkLabel(frame, text = 'Please Sign-Up!', font = ("Times", 75, "bold"), text_color = 'Black')
    label2.place(x = framex / 2- 230 - 15, y =10)
    spaces = '       '
    label3 = CTkLabel(frame, text = "Username Below", font = ("Times", 45, "bold"), text_color = 'Black').place(x = framex / 2 - 200 + 25, y = 200 - 40)
    entry1 = CTkEntry(frame, placeholder_text = f'{spaces}Username Here', width = 360, font = ("Times", 35, "bold"))
    entry1.place(x = framex / 2 - 200, y = 275 - 40)
...
while True:
    if flag == 'Quit':
        break
    if flag == 'Go to login'.lower():
        flag = ''
        Login()
        continue
    if flag == 'Go to Landing'.lower():
        flag = ''
        Landing()
        continue
    if flag =='Go to SignUp'.lower():
        flag = ''
        SignUp()
        continue
    else:
